...
{% block content %}
    <form action="{{ urlFor('register.post') }}" method="post" autocomplete="off">
    	...
    	<div>
    		...
    		<input type="password" name="password" id="password"{% if request.post('password') %} value="{{ request.post('password') }}"{% endif %}>
    		{% if errors.first('password') %}{{ errors.first('password') }} {% endif %}
    	</div>
    	<div>
    		<label for="password_confirm">Confirm Password
    		</label> 
    		<input type="password" name="password_confirm" id="password_confirm"{% if request.post('password_confirm') %} value="{{ request.post('password_confirm') }}"{% endif %}>
