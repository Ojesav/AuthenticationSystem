...
use Carbon\Carbon;
	...
	$app->get('/login',$guest(),function() use 
		($app){
			$app->render('auth/login.php');
	})->name('login');

	$app->post('/login',$guest(),function() use 
		($app){
		...
		if($v->passes())
		{
			...
			if($user && $app->hash->passwordCheck($password,$user->passwd))
			{
				...
				if ($remember=='on'){
					...
					$user->updateRememberCredentials($rememberIdentifier,
